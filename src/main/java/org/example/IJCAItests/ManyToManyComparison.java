...
public class ManyToManyComparison {
    public static void main(String[] args) throws DirectedAcyclicGraph.CycleFoundException, IOException, ContradictionException
    {
        ...
        long timeLimit = Integer.parseInt(args[3]) * 1000;
        String timeLimit_string = args[3] + 's';
        //int cutOffLimit = 1000;
        int kfoldValue = Integer.parseInt(args[2]);
        int[] N = {Integer.parseInt(args[0])};
        int[] C = {Integer.parseInt(args[1])};

        FileWriter pw = new FileWriter(new File("comparison_MM_" + N[0] + "_" + C[0] + "_"  + kfoldValue + "_" + timeLimit_string + ".csv"));
        pw.append("N-C");
        pw.append(',');
        pw.append("PP1");
        pw.append(',');
        ...
        for (int i = 0; i < N.length; i++) {
            System.out.println("Size : " + N[i] + "-" + C[i]);
            averagePP1 = 0;
            averagePP2 = 0;
            averageV = 0;
            averageB = 0;
